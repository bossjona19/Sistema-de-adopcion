...
**Slice 2 — ✅ COMPLETADO**
- [x] Dashboard: tarjeta con **casos en seguimiento** + **próximas visitas** (vencidas en rojo) → `getSeguimientoResumen`
- [x] **PDF de seguimiento** por caso (botón en la pestaña Post-adopción)
- [x] Anti doble-submit en post-adopción (flag) y notas (botón) — bug de doble-click reportado
...
- [x] **Branding global (white-label):** el nombre de la ONG reemplaza "Proyecto OMEGA"/"OMEGA" en login, landing, solicitud, 404, dashboard (breadcrumb/sidebar), **título del navegador**, footers, textos de consentimiento y reportes → `branding.js` (`applyBranding` + atributos `data-org`)

**Archivos:** `services/configService.js`, `features/config.js`, `branding.js`, `dashboard.html`, `sidebar.js`, `index/login/solicitud/404.html`, `docs/fase_b7_organizacion.sql` · **DB:** tabla `organizacion` · **Esfuerzo:** M
...
- [x] Sin paginación (>200 registros) → **resuelto en B4** (paginación server-side + índices)
- [ ] PWA icons solo SVG (iOS) → pendiente menor (cosmético)
...
| 2026-06-03 | **Hardening de cierre (prod-ready)** | 🟢 **CSP** + Permissions-Policy en `vercel.json`. **Recuperación de contraseña** (Supabase): `recuperar.html` + `restablecer.html` + helpers en `auth.js` + link en login. **CI/CD**: `package.json` (type:module) + `.github/workflows/ci.yml` (node --check de todo + `node --test`). **Testing**: `test/ui.test.js` (7 pruebas de lógica pura, runner nativo, 0 deps). **Limpieza**: `escapeHtml` centralizado, `getAll()` muertos y `cards.js` eliminados. SW→v44 | Usuario: añadir `/restablecer.html` a Redirect URLs de Supabase; verificar deploy con CSP en preview |
...
| 2026-06-03 | **Limpieza (auditoría)** | 🟢 Tras auditoría de cierre: eliminado **código muerto** (`getAll()` x3 sin uso) y **`escapeHtml` centralizado** en `core/ui.js` (3 copias unificadas). Sin cambio de comportamiento. SW→v42 | — |
| 2026-06-04 | **Lenguaje institucional / terminología** | 🧭 **Solo presentación (sin SQL).** Estado del menor: Disponible→"En espera de adopción", En proceso→"En proceso de adopción". Preferencia de edad → **etapas de desarrollo** (Bebé/Niñez/Adolescencia) con mapeo de compatibilidad para registros antiguos. Pregunta del formulario reformulada (compatibilidad familiar, no selección). Maps `ESTADO_MENOR_LABELS`/`PREFERENCIA_EDAD_LABELS` en `ui.js`. Claves de BD intactas. SW→v43 | — |
| 2026-06-05 | **README + fix CSP fonts** | 🟢 `README.md` reescrito al estado real (badge CI, características, setup, roles, docs, autor). **Fix CSP:** allowlist de Google Fonts (`fonts.googleapis.com` en style-src, `fonts.gstatic.com` en font-src) en `vercel.json`. | Redeploy para aplicar el header CSP nuevo |
